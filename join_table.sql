...
select fname, age from dataofcustomer where age = 18;

select max(age)  from dataofcustomer;

select fname, age from dataofcustomer where age = 90;
...
select count(fname), city from dataofcustomer group by 2;
...
select distinct(astatus), count(*)  from account group by 1;
